...
foreach($document in $documents|Where-Object {$_-like'docs/*'-and$_-ne'docs/README.md'}) {
    Check (@($links|Where-Object {$_.from-eq'docs/README.md'-and$_.to-eq$document}).Count-gt0) "Document is missing from docs/README.md: $document"
}
...
foreach($file in $files|Where-Object {$_-notlike'Artifacts/*'-and$_-ne'conformance/Invoke-DocumentationGate.ps1'-and$_-match'\.(md|ps1|mjs|ts|java|json|properties|gradle|kts|ya?ml)$'}) {
    ...
    if($file-eq'docs/architecture/adr/0004-v1-loopback-release-profile.md'){$content=$content -replace '(?m)^- Archive identity:.*$',''}
...
$schema=Get-Content (Join-Path $repo 'protocol-schema/src/main/openapi/minecraft-control-v0.json') -Raw|ConvertFrom-Json
$server=Get-Content (Join-Path $repo 'companion/src/server.ts') -Raw
...
$reference=Get-Content (Join-Path $repo 'docs/reference/protocol-v0.md') -Raw
$companion=Get-Content (Join-Path $repo 'companion/README.md') -Raw
...
$vision=Get-Content (Join-Path $repo 'docs/product/vision.md') -Raw
$control=Get-Content (Join-Path $repo 'docs/architecture/agent-control.md') -Raw
$pack=Get-Content (Join-Path $repo 'docs/testing/modpack-compatibility.md') -Raw
