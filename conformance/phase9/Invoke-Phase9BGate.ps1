...
$revisionIdentityStatic=& (Join-Path $PSScriptRoot 'Invoke-Phase9B2StaticGate.ps1');if($revisionIdentityStatic.Result-ne'PASS'){throw'9B.2 static gate failed'}
...
if(-not $SkipLive){
    ...
    $revisionIdentity=& (Join-Path $PSScriptRoot 'Invoke-Phase9B2Gate.ps1') @p -SkipBuild
