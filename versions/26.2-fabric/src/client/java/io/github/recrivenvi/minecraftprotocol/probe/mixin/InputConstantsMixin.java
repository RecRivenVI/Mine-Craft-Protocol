...
abstract class InputConstantsMixin {
    ...
    private static void minecraftProtocolProbe$noHostWarp(Window window, int mode, double x, double y, CallbackInfo ci) {
        if (FabricProbeRuntime.isAgentControlActive()) ci.cancel();
    }
    @Inject(method = "isKeyDown", at = @At("HEAD"), cancellable = true)
    private static void minecraftProtocolProbe$virtualPolling(Window window, int key, CallbackInfoReturnable<Boolean> cir) {
        if (FabricProbeRuntime.isAgentControlActive()) cir.setReturnValue(FabricProbeRuntime.isAgentKeyDown(key));
    }
}
