...
> Authority: `documents/product/vision.md` defines the committed Core, `documents/product/extensions.md` defines optional extensions, and this file records the current implemented Runtime and Companion architecture.
...
Exclusive input, separated host cursor, virtual pointer and pixel Chrome are implemented together in control-r24. Automated development checks do not constitute unified human/visual acceptance, which remains pending. See `documents/architecture/agent-control.md`. Historical UX closeout evidence remains `validations/results/core/core-ux-closeout-20260905.json`; the isolated Forge save-click timeout is retained as a historical observation, not erased by later passes.
