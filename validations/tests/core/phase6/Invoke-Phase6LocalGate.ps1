...
try {
    ...
    $schemaPath = Join-Path $repositoryRoot 'components\protocol-schema\src\main\openapi\minecraft-control-v0.json'
    ...
    Assert-True (Test-Path -LiteralPath (Join-Path $repositoryRoot 'validations\tests\core\phase6\Invoke-Phase6PeerConformance.ps1')) 'Integrated Peer conformance must exist'
    Assert-True (Test-Path -LiteralPath (Join-Path $repositoryRoot 'validations\tests\core\phase6\Invoke-Phase6DedicatedPeerConformance.ps1')) 'Dedicated Peer conformance must exist'
    ...
    $fabricBuild = Get-Content -LiteralPath (Join-Path $repositoryRoot 'versions\26.2-fabric\build.gradle.kts') -Raw
    ...
    [pscustomobject]@{
        ...
        JavaModels = @(Get-ChildItem -LiteralPath (Join-Path $repositoryRoot 'components\protocol-schema\build\generated\java') -Recurse -Filter '*.java').Count
        TypeScriptFiles = @(Get-ChildItem -LiteralPath (Join-Path $repositoryRoot 'components\protocol-schema\build\generated\typescript') -Recurse -Filter '*.ts').Count
        Targets = $artifacts
    }
}
finally {
