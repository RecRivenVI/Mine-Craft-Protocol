...
function Assert-True {
    ...
    if (-not $Condition) {
        throw "Phase 4 local gate assertion failed: $Message"
    }
}

$tasks = @(
    ':components:protocol-schema:openApiValidate',
    ':components:protocol-schema:generateProtocol',
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Phase 4 Gradle gate failed with exit code $LASTEXITCODE"
    }

    $schemaPath = Join-Path $repositoryRoot 'components\protocol-schema\src\main\openapi\minecraft-control-v0.json'
    $schema = Get-Content -LiteralPath $schemaPath -Raw | ConvertFrom-Json
    Assert-True ($schema.info.version -match '^0\.0\.1-phase([4-9]|[1-9][0-9]+)$') 'OpenAPI version must retain Phase 4 or later behavior'
    ...
    $artifacts = foreach ($target in @(
        ...
        Assert-True ($jar.Name -match 'phase([4-9]|[1-9][0-9]+)') "$target artifact must carry Phase 4 or later version"
        [pscustomobject]@{
            Target = $target
            Artifact = $jar.Name
            Bytes = $jar.Length
    ...
    $javaModels = @(Get-ChildItem -LiteralPath (Join-Path $repositoryRoot 'components\protocol-schema\build\generated\java') -Recurse -Filter '*.java')
    $typeScriptModels = @(Get-ChildItem -LiteralPath (Join-Path $repositoryRoot 'components\protocol-schema\build\generated\typescript') -Recurse -Filter '*.ts')
