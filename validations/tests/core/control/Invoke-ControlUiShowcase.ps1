...
$script:instance=Join-Path $repo "validations/.local/control-ui-showcase/$Target"
...
try {
    New-Item -ItemType Directory -Path (Join-Path $repo 'validations/.local') -Force|Out-Null
    # One Runner, one target at a time. A stale empty lock file grants no ownership.
    $runnerLock=[IO.File]::Open((Join-Path $repo 'validations/.local/control-ui-showcase.runner.lock'),[IO.FileMode]::OpenOrCreate,[IO.FileAccess]::ReadWrite,[IO.FileShare]::None)
