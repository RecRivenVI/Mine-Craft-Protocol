...
Push-Location $root
try {
    $static = & '.\validations\tests\core\phase9\Invoke-Phase9D0StaticGate.ps1'
    ...
    Push-Location (Join-Path $root 'components/companion')
    try {
    ...
    $phase9bStatic = @(
        & '.\validations\tests\core\phase9\Invoke-Phase9BStaticGate.ps1'
        & '.\validations\tests\core\phase9\Invoke-Phase9B1StaticGate.ps1'
        & '.\validations\tests\core\phase9\Invoke-Phase9B2StaticGate.ps1'
    )
    Assert-True ($phase9bStatic.Result -notcontains 'FAIL') 'Phase 9B static regression failed'
    $phase9cStatic = & '.\validations\tests\core\phase9\Invoke-Phase9CStaticGate.ps1'
    Assert-True ($phase9cStatic.Result -eq 'PASS') 'Phase 9C static regression failed'

    $live = 'SKIPPED'
    $liveResult = $null
    if (-not $SkipLive) {
        $liveResult = & '.\validations\tests\core\phase9\Invoke-Phase9D0FiveTargetLiveGate.ps1' -Offline:$Offline
