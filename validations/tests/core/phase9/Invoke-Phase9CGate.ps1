...
if(-not$SkipLive){if(-not$BaseUri-or-not$TokenFile-or-not$ExpectedTarget){throw'Live parameters required'};$live=& (Join-Path $PSScriptRoot 'Invoke-Phase9CTargetConformance.ps1') -BaseUri $BaseUri -TokenFile $TokenFile -ExpectedTarget $ExpectedTarget -SkipBatch:$SkipBatch;if($live.Result-ne'PASS'){throw'Phase 9C live failed'};$phase9b=& (Join-Path $PSScriptRoot 'Invoke-Phase9BGate.ps1') -BaseUri $BaseUri -TokenFile $TokenFile -ExpectedTarget $ExpectedTarget -SkipBuild;if($phase9b.Result-ne'PASS'){throw'Phase 9B live regression failed'};$phase9bLive='PASS';$smoke=& (Join-Path $root 'validations\tests\releases\phase8\Invoke-Phase8TargetSmoke.ps1') -BaseUri $BaseUri -TokenFile $TokenFile -ExpectedTarget $ExpectedTarget -ExpectedBackend any -EnterWorld -RequireAuthoritative;if($smoke.Result-ne'PASS'){throw'V1 live regression failed'};$v1='PASS'}
