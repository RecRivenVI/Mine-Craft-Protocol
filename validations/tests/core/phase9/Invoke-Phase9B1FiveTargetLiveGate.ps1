...
try{
 ...
 $results=foreach($run in $runs){
  ...
  $b1=& '.\validations\tests\core\phase9\Invoke-Phase9B1Gate.ps1' -BaseUri $base -TokenFile $tokenFile -ExpectedTarget $run.Target -HasPhase9ADiagnostics:$run.Has9A -SkipBuild
