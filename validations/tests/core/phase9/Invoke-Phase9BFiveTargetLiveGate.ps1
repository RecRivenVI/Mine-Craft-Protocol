...
try{
 ...
 $results=foreach($run in $runs){
  ...
  $gate=& '.\validations\tests\core\phase9\Invoke-Phase9BGate.ps1' -BaseUri $base -TokenFile $tokenFile -ExpectedTarget $run.Target -SkipBuild -HasPhase9ADiagnostics:$run.Has9A
