...
try {
    ...
    $file = Join-Path $root 'components/runtime-safety/src/main/java/io/github/recrivenvi/minecraftprotocol/safety/PersistentWriteSafetyFoundation.java'
    ...
    foreach ($target in $targets) {
        $build = Get-Content -LiteralPath (Join-Path $root "versions/$target/build.gradle.kts") -Raw
        Assert-True ($build.Contains('project(":components:runtime-safety")')) "$target must consume the shared safety foundation"
        $hashes[$target] = $hash
    }
    ...
    $openApi = Get-Content -LiteralPath (Join-Path $root 'components/protocol-schema/src/main/openapi/minecraft-control-v0.json') -Raw
