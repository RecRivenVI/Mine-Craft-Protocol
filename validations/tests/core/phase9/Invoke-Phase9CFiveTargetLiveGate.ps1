...
$runs=@(
 ...
 @{Target='26.2-neoforge';Task=':versions:26.2-neoforge:runClient';Dir='instances\26.2-neoforge\client';Port=25582;Has9A=$true;WorldExit=$true},
...
try {
 ...
 $results=foreach($run in $runs){
  ...
  $phase9c=& '.\validations\tests\core\phase9\Invoke-Phase9CTargetConformance.ps1' -BaseUri $base -TokenFile $tokenFile -ExpectedTarget $run.Target
  Assert-True($phase9c.Result-eq'PASS')"$($run.Target) Phase 9C"
  $worldExit='NOT_REPRESENTATIVE'
  $maximumBatch='NOT_REPRESENTATIVE'
  if($run.WorldExit){Start-Sleep -Seconds 5;$maximum=& '.\validations\tests\core\phase9\Invoke-Phase9CMaximumBatchConformance.ps1' -BaseUri $base -TokenFile $tokenFile -ExpectedTarget $run.Target;Assert-True($maximum.Result-eq'PASS')'maximum batch representative';$maximumBatch='PASS';Start-Sleep -Seconds 5;$world=& '.\validations\tests\core\phase9\Invoke-Phase9CWorldExitConformance.ps1' -BaseUri $base -TokenFile $tokenFile -ExpectedTarget $run.Target;Assert-True($world.Result-eq'PASS')'world exit representative';$worldExit='PASS'}
  Start-Sleep -Seconds 5
  $phase9b=& '.\validations\tests\core\phase9\Invoke-Phase9BGate.ps1' -BaseUri $base -TokenFile $tokenFile -ExpectedTarget $run.Target -HasPhase9ADiagnostics:$run.Has9A -SkipBuild
  Assert-True($phase9b.Result-eq'PASS')"$($run.Target) Phase 9B regression"
  $v1=& '.\validations\tests\releases\phase8\Invoke-Phase8TargetSmoke.ps1' -BaseUri $base -TokenFile $tokenFile -ExpectedTarget $run.Target -ExpectedBackend any -EnterWorld -RequireAuthoritative
