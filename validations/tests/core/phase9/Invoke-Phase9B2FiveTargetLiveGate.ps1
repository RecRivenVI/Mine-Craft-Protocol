...
function A([bool]$condition,[string]$message){if(-not $condition){throw "Phase 9B.2 five-target live failed: $message"}}
...
$runs=@(
 @{Target='1.20.1-forge';Task=':versions:1.20.1-forge:runClient';Dir='instances\1.20.1-forge\client';Port=25581;Has9A=$true},
 @{Target='1.21.1-neoforge';Task=':versions:1.21.1-neoforge:runClient';Dir='instances\1.21.1-neoforge\client';Port=25581;Has9A=$false},
 @{Target='26.1.2-neoforge';Task=':versions:26.1.2-neoforge:runClient';Dir='instances\26.1.2-neoforge\client';Port=25582;Has9A=$false},
 @{Target='26.2-neoforge';Task=':versions:26.2-neoforge:runClient';Dir='instances\26.2-neoforge\client';Port=25582;Has9A=$true},
 @{Target='26.2-fabric';Task=':versions:26.2-fabric:runClient';Dir='instances\26.2-fabric\client';Port=25583;Has9A=$true})
Push-Location $root
try{
 $env:MCP_RUNTIME_SCOPES='read,ui,input,capture,event,diagnostics,control,command,fixture,debug'
 $results=foreach($run in $runs){
  Write-Host "[Phase9B.2] starting $($run.Target)"
  ...
  $process=Start-Process '.\gradlew.bat' -ArgumentList $run.Task,'--no-daemon','--offline' -WorkingDirectory $root -RedirectStandardOutput(Join-Path $dir 'phase9b2-live-stdout.log') -RedirectStandardError(Join-Path $dir 'phase9b2-live-stderr.log') -PassThru -WindowStyle Hidden
  ...
  $b2=& '.\validations\tests\core\phase9\Invoke-Phase9B2Gate.ps1' -BaseUri $base -TokenFile $tokenFile -ExpectedTarget $run.Target -SkipBuild
  A($b2.Result-eq'PASS')"$($run.Target) 9B.2"
  $p=@{BaseUri=$base;TokenFile=$tokenFile;ExpectedTarget=$run.Target}
  $formal=[ordered]@{Deep=& '.\validations\tests\core\phase9\Invoke-Phase9BDeepObservationConformance.ps1' @p -HasPhase9ADiagnostics:$run.Has9A;ClientServer=& '.\validations\tests\core\phase9\Invoke-Phase9BClientServerConformance.ps1' @p;Provider=& '.\validations\tests\core\phase9\Invoke-Phase9BProviderConformance.ps1' @p;Tickets=& '.\validations\tests\core\phase9\Invoke-Phase9BTicketConformance.ps1' @p;Scheduled=& '.\validations\tests\core\phase9\Invoke-Phase9BScheduledTickConformance.ps1' @p;Budget=& '.\validations\tests\core\phase9\Invoke-Phase9BBudgetConformance.ps1' @p}
  foreach($entry in $formal.GetEnumerator()){A($entry.Value.Result-eq'PASS')"$($run.Target) $($entry.Key)"}
  $v1=& '.\validations\tests\releases\phase8\Invoke-Phase8TargetSmoke.ps1' -BaseUri $base -TokenFile $tokenFile -ExpectedTarget $run.Target -ExpectedBackend any -EnterWorld -RequireAuthoritative;A($v1.Result-eq'PASS')"$($run.Target) V1"
  ...
  Write-Host "[Phase9B.2] completed $($run.Target)"
  [pscustomobject]@{Target=$run.Target;Canonical=$b2.Details.Canonical.Result;ProviderRevision=$b2.Details.Provider.Result;Epoch=$b2.Details.Resource.Epoch;Lifecycle=$b2.Details.Resource.LifecycleFields;Executors=$b2.Details.Executors.Result;Formal='PASS';V1='PASS';Shutdown='PASS';Metrics=$formal.Budget}
 }
