...
Push-Location $root
try {
    $static = & '.\validations\tests\core\phase9\Invoke-Phase9AStaticGate.ps1'
    ...
    if (-not $SkipLive) {
        ...
        $live = [ordered]@{
            Observation = & '.\validations\tests\core\phase9\Invoke-Phase9ADeepObservationConformance.ps1' @parameters
            Debug = & '.\validations\tests\core\phase9\Invoke-Phase9ADebugSpikeConformance.ps1' @parameters
            Storage = & '.\validations\tests\core\phase9\Invoke-Phase9AStorageReadConformance.ps1' @parameters
        }
        # Debug mutations share the bounded expensive-operation bucket. Let it refill before the reconstruction scenario.
        Start-Sleep -Seconds 5
        $live['Reconstruction'] = & '.\validations\tests\core\phase9\Invoke-Phase9AReconstructionConformance.ps1' @parameters
