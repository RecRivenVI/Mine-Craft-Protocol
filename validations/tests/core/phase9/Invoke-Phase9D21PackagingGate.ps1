...
$targets = @(
    @{ Id = '1.20.1-forge'; BuildFile = 'versions/1.20.1-forge/build.gradle.kts'; Package = 'jar'; NestedPrefix = 'META-INF/jarjar/'; DevRuntime = $true },
    @{ Id = '1.21.1-neoforge'; BuildFile = 'versions/1.21.1-neoforge/build.gradle.kts'; Package = 'jar'; NestedPrefix = 'META-INF/jarjar/'; DevRuntime = $true },
    @{ Id = '26.1.2-neoforge'; BuildFile = 'versions/26.1.2-neoforge/build.gradle.kts'; Package = 'jar'; NestedPrefix = 'META-INF/jarjar/'; DevRuntime = $false },
    @{ Id = '26.2-neoforge'; BuildFile = 'versions/26.2-neoforge/build.gradle.kts'; Package = 'jar'; NestedPrefix = 'META-INF/jarjar/'; DevRuntime = $false },
    @{ Id = '26.2-fabric'; BuildFile = 'versions/26.2-fabric/build.gradle.kts'; Package = 'build'; NestedPrefix = 'META-INF/jars/'; DevRuntime = $false }
)
$sharedClass = 'io/github/recrivenvi/minecraftprotocol/safety/PersistentWriteSafetyFoundation.class'

Push-Location $root
try {
    $foundationPath = Join-Path $root 'components/runtime-safety/src/main/java/io/github/recrivenvi/minecraftprotocol/safety/PersistentWriteSafetyFoundation.java'
    Assert-True (Test-Path -LiteralPath $foundationPath) 'shared runtime-safety source is missing'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $root 'components/runtime-safety/src/main/java/io/github/recrivenvi/minecraftprotocol/probe/runtime/PersistentWriteSafetyFoundation.java')) ) `
        'shared safety source must not remain in the target runtime package'

    foreach ($target in $targets) {
        $buildPath = Join-Path $root $target.BuildFile
        $build = Get-Content -LiteralPath $buildPath -Raw
        Assert-True ($build.Contains('implementation(project(":components:runtime-safety"))')) "$($target.Id) compile dependency missing"
        if ($target.Id -eq '26.2-fabric') {
            Assert-True ($build.Contains('include(project(":components:runtime-safety"))')) "$($target.Id) Loom include missing"
        } else {
            Assert-True ($build.Contains('jarJar(project(":components:runtime-safety"))')) "$($target.Id) Jar-in-Jar dependency missing"
            if ($target.DevRuntime) {
                Assert-True ($build.Contains('add("additionalRuntimeClasspath", project(":components:runtime-safety"))')) "$($target.Id) development runtime classpath missing"
            } else {
                Assert-True (-not $build.Contains('add("additionalRuntimeClasspath", project(":components:runtime-safety"))')) "$($target.Id) must use ModDev's normal runtime classpath"
            }
        }
    }
