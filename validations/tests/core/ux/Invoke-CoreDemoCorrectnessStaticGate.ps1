...
$client=Get-Content (Join-Path $root 'components/companion/src/runtime-client.ts') -Raw
$result=Get-Content (Join-Path $root 'components/companion/src/result.ts') -Raw
...
$hookGate=& (Join-Path $root 'validations/tests/compatibility/phase7/Invoke-Phase7HookCompatibilityGate.ps1')
