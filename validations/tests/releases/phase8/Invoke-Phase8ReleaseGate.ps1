...
try {
    ...
    $local = & '.\validations\tests\releases\phase8\Invoke-Phase8LocalGate.ps1' @localArguments
    ...
    $cancellation = & '.\validations\tests\releases\phase8\Invoke-Phase8CancellationConformance.ps1' `
    ...
    $events = & '.\validations\tests\releases\phase8\Invoke-Phase8EventConformance.ps1' `
        -BaseUri $representative.baseUri -TokenFile $representative.tokenFile -ExpectedTarget $representative.target
    Assert-True ($events.Result -eq 'PASS') 'WebSocket reliability conformance failed'
    $security = & '.\validations\tests\releases\phase8\Invoke-Phase8SecurityConformance.ps1' `
        -BaseUri $representative.baseUri -TokenFile $representative.tokenFile -ExpectedTarget $representative.target
    Assert-True ($security.Result -eq 'PASS') 'security conformance failed'
    $conditions = & '.\validations\tests\releases\phase8\Invoke-Phase8ConditionConformance.ps1' `
        -BaseUri $representative.baseUri -TokenFile $representative.tokenFile -ExpectedTarget $representative.target
    Assert-True ($conditions.Result -eq 'PASS') 'typed Wait/Assert conformance failed'
    $recording = & '.\validations\tests\releases\phase8\Invoke-Phase8RecordingHardeningConformance.ps1' `
        -BaseUri $representative.baseUri -TokenFile $representative.tokenFile -ExpectedTarget $representative.target
