...
$client = Get-Content -LiteralPath (Join-Path $root 'components\companion\src\runtime-client.ts') -Raw
$server = Get-Content -LiteralPath (Join-Path $root 'components\companion\src\server.ts') -Raw
...
$conditionConformance = Get-Content -LiteralPath `
    (Join-Path $root 'validations\tests\releases\phase8\Invoke-Phase8ConditionConformance.ps1') -Raw
...
$schema = Get-Content -LiteralPath (Join-Path $root 'components\protocol-schema\src\main\openapi\minecraft-control-v0.json') -Raw | ConvertFrom-Json
