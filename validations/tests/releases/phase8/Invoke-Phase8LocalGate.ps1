...
$companion = Join-Path $repositoryRoot 'components/companion'
...
$tasks = @(':components:protocol-schema:openApiValidate', ':components:protocol-schema:generateProtocol')
...
try {
    ...
    $schema = Get-Content 'components\protocol-schema\src\main\openapi\minecraft-control-v0.json' -Raw | ConvertFrom-Json
    ...
    $hardening = & '.\validations\tests\releases\phase8\Invoke-Phase8HardeningStaticGate.ps1'
    Assert-True ($hardening.Result -eq 'PASS') 'Phase 8 hardening static gate must pass'
    Assert-True (Test-Path -LiteralPath '.\validations\tests\releases\phase8\Invoke-Phase8RemoteParityGate.ps1') `
        'Phase 8 Remote Parity Gate must exist'
    $auditClassifierTests = & '.\validations\tests\releases\phase8\Invoke-Phase8DependencyAuditClassifierTests.ps1'
    ...
    $productionFiles = @(
        'components\companion\src\index.ts',
        'components\companion\src\server.ts',
        'components\companion\src\config.ts',
        'components\companion\src\runtime-client.ts',
        'components\companion\src\result.ts',
        'components\companion\src\session-state.ts'
    )
    ...
    $lock = Get-Content 'components\companion\package-lock.json' -Raw | ConvertFrom-Json -AsHashtable
    ...
    [pscustomobject]@{
        ...
        JavaModels = @(Get-ChildItem 'components\protocol-schema\build\generated\java' -Recurse -Filter '*.java').Count
        TypeScriptFiles = @(Get-ChildItem 'components\protocol-schema\build\generated\typescript' -Recurse -Filter '*.ts').Count
    }
}
finally {
    Pop-Location
